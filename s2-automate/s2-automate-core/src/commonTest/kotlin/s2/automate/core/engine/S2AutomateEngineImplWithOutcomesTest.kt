...
/**
 * Tests for the WithOutcomes paths in S2AutomateEngineImpl:
 ...
 */
class S2AutomateEngineImplWithOutcomesTest {
    ...
    /**
     * Outcome pattern for scripting — Committed uses the actual context event;
     * failure variants carry fixed error codes.
     */
     ...
     * event is used so the engine's `find { it.event == outcome.event }` lookup succeeds.
     */
    private class StubPersister(
        ...
        override suspend fun persistWithOutcomes(
            ...
            val ctxs = transitionContexts.toList()
            return ctxs.mapIndexed { i, ctx ->
                toOutcome(transitionPattern[i % transitionPattern.size], ctx.event)
            }.asFlow()
        }
    ...
    private fun makeEngine(
        ...
        return S2AutomateEngineImpl(automateContext, guard, persister, automateEventPublisher)
    }
    ...
    fun `guards fire for every command in createWithOutcomes`() = runTest {
        ...
        // evaluateInit fires once per command (in prepareCreationContext)
    ...
    fun `sendEndDoTransitionEvent only fires for Committed outcomes`() = runTest {
        ...
        assertEquals(
            ...
            "AutomateTransitionEnded should fire exactly once per Committed outcome; published=${publisher.published.map { it::class.simpleName }}"
        )
    }
